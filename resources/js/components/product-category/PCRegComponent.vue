...
<div>
    ...
    <form @submit.prevent="handleSubmit" @change="emptyField()">
      ...
      <div class="input-group">
        ...
        <input :disabled="loading !== 'neutral'" id="name" name="name" type="text" :class="{'is-invalid' : nameErrors.length > 0, 'is-inv' : nameErrors.length > 0, 'form-control':true }" placeholder="Cosmetics" aria-label="name" v-model="name" @input="disabling(name, nameErrors )">
      </div>
      ...
      if(this.button_clicked && this.nameErrors.length < 1) {
        ...
        .then(response => {
          ...
          if(response.status === 201) {
            ...
            setTimeout(() => window.location.replace("/product-categories"), 2000);
          } else{
            setTimeout(() => {
              Alt.alternative({
                status:'error',
