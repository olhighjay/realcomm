    ...
    <div class="container" id="table">
      ...
      <div class="row justify-content-center">
        <div class="col-md-6 text-center mb-5">
          <h2 class="heading-section">Business Categories</h2>
        </div>
      </div>
      <div class="row justify-content-center">
        <div class="col-md-6">
          <div class="col-md-6">
            <!-- start search -->
            <search :modelUrl="'/business/categories'" @updatedSearchedData="getBCSearchResults($event)"></search>
            <!-- end search -->
          </div>
        </div>
        ...
        <div class="col-md-12">
          <div class="table-wrap" style="overflow-x:scroll;">
                  ...
                  <div class="dropdown show d-inline-block widget-dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown-recent-order1">
                      <li class="dropdown-item">
                        <a :href="'/business-categories/'+category.id+'/edit'">View</a>
                      </li>
                      <li class="dropdown-item">
                        <a href="#" @click="BCConfirmDeactivate([category.id], category.status === 'inactive' ? 'activate' : 'deactivate', 'single-update')">{{category.status === 'inactive' ? 'Activate' : 'Deactivate'}} </a> 
                      </li>
                      <li class="dropdown-item">
                        <a href="#" @click="BCConfirmDeactivate([category.id], 'delete', 'single-update')">Remove</a>
                      </li>
                    </ul>
                  </div>
                </td>
            ...
            <div class="row justify-content-center">
              <div class="col-md-6 mb-2 pt-2">
                <div v-if="selected.length > 0" class="btn-group mb-1">
                  ...
                  <div class="dropdown-menu">
                    <a @click=" BCConfirmDeactivate(selected, 'activate', 'mass-update')" class="action-item dropdown-item" href="#" >Activate Selected Records</a>
                    <a  @click="BCConfirmDeactivate(selected, 'deactivate', 'mass-update')" class="dropdown-item" href="#">Deactivate Selected Records</a>
                    <a  @click="BCConfirmDeactivate(selected, 'delete', 'mass-update')" class="dropdown-item" href="#" >Delete Selected Records</a>
                  </div>
                  &nbsp;  &nbsp;  &nbsp;
                </div>
              ...
              <div class="col-md-6 text-center d-flex mb-5 justify-content-end">
                <Pagination :incomingData="pageComp" :searchQuery="searchQuery" :modelUrl="'/business/categories'" @updatePaginatedData="getBCPaginationResults($event)"></Pagination>
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  import Pagination from './BCPaginationComponent.vue'
  ...
  import Filtering from './BCFilterByComponent.vue'
...
export default {
  ...
  methods: {
    ...
    orderObject(name, direction) {
      ...
      axios.get(`/business/categories?name=${name}&&direction=${direction}&&search=${this.searchQuery}&&page=1`)
        .then(response => {
          this.categories = response.data.businessCategories.data;
          this.pageComp = response.data.businessCategories
          if(name === 'name'){
    ...
    BCConfirmDeactivate(ids, type, update_mode){
      Alt.alternative({
        status:'question',
      ...
      .then((res) => {
        if (res){
          ...
          if(update_mode === 'mass-update') {
            updateBCUrl = '/business-categories/mass-update';
          } else if(update_mode === 'single-update') {
            updateBCUrl = `/business-categories/${ids[0]}/edit`;
          }
          // Update the selected records
          axios.post(updateBCUrl, {type: type, ids: ids})
          .then(response => {
            if(response.status === 200) {
              // Update the index page data
              axios.get('/business/categories')
                .then(response => {
                  Alt.alternative({
                    status:'success',
                    title:'Success',
    ...
    getBCPaginationResults(results) {
      this.categories = results;
    },

    getBCSearchResults(results) {
      this.categories = results[0].businessCategories.data;
      this.pageComp = results[0].businessCategories;
